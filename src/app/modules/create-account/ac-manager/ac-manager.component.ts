...
@Component({
  selector: 'app-ac-manager',
  templateUrl: './ac-manager.component.html',
  styleUrls: ['./ac-manager.component.scss']
})
export class AcManagerComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.managerAccountForm = this.fb.group({
  ...
  next() {
    ...
    if (this.step == 1) {
      // if (this.managerAccountForm.controls.name.valid, this.managerAccountForm.controls.password.valid,
      //   this.managerAccountForm.controls.email.valid, this.managerAccountForm.controls.cnfPassword.valid) {
  ...
  onSubmit() {
    console.log('managerAccountForm details', this.managerAccountForm.value);

  }
  get f() {
    return this.managerAccountForm.controls;
  }
